...
layout(rgba32f, binding = 0) uniform image2D screenBuffer;
...
layout(std430, binding = 4) buffer nodesData {
    Node nodes[];
};
...
layout(location = 4) uniform vec3 front;
layout(location = 5) uniform vec3 right;
...
//     // s.spheres[numSpheres++] = sphere(vec3(-4, 1, 0), 1.0, dielectric(1.4)); // Big Metal Sphere
...
//     s.spheres[numSpheres++] = sphere(vec3(-1.6, 0.2, 2.5), 0.2, lambertian(vec3(1.0, 0.6, 0.6))); 
...
HitRecord traceRay(in Ray r)
{
    ...
    while(true) {
        int sphereCount = node.count;
        int content = node.content;
        
        if (sphereCount > 0) {
            
            for(int i = 0; i < sphereCount; i++) {
                int sphereIdx = sphereIDs[i + content];
                if (hitSphere(spheres[sphereIdx], r, interval(0.001, nearestHit), rec)){
                    hitSomething = true;
                    nearestHit = rec.t;
                }
            }
            ...
            Node leftChild = nodes[content];
            Node rightChild = nodes[content + 1];

            float dist1 = hit(r, leftChild, nearestHit);
            float dist2 = hit(r, rightChild, nearestHit);
    ...
    if(!hitSomething){
        vec3 unit_direction = normalize(r.dir);
        float a = 0.5*(unit_direction.y + 1.0);
        rec.color *= (1.0-a)*vec3(1.0) + a*vec3(0.5, 0.7, 1.0);
    }
    else {
        if (scatter(r, rec, attenuation)) {
            rec.bounce = true;
...
{    
    ...
    ivec2 dims = imageSize(screenBuffer);
    ...
    vec3 direction = front + x*right + y*up*dims.y/dims.x;
    ...
    for(int i = 0; i < MAX_BOUNCES; i++)
    {
        ...
        pixelColor *= rec.color;

        if (!rec.hit || !rec.bounce) break;
    }

    if (rec.hit) pixelColor = vec3(0.0);
    
    // vec3 pixelColor = rayColor(r, scene);
    // vec3 pixelColor = traceRay2(r);
    ...
    imageStore(screenBuffer, pixelCoords, vec4(pixelColor,1.0));
